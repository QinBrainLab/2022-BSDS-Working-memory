...
% [datao,stim_design] = combine_data();
% for subj = 1:length(datao)
%     data{subj} = datao{subj}([1:2],:);
% end
...
VBprior.no = M + 2;
% VBprior.Wo = pinv(0.75*cov(cell2mat(data)'));
...
for iter = 1:nIter
      ...
      for k = 1:K
            if iter == 1
                  barAlpha = (VBprior.ao)/(VBprior.bo); %from the previous step
                  Lambda = VBprior.Wo;
            else
